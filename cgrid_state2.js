...
# ---------------------------------
*/
Ext.define('canopsis.lib.view.cgrid_state2' , {
	...
	initComponent: function() {
		...
		// Agrego la columna de Wiki
        if (this.opt_show_wiki) {
            this.columns.push({
                header: 'H',
                sortable: this.opt_column_sortable,
                width: 40,
                renderer: function(value, metaData, record, row, col, store, gridView){
                	return this.salidaHTML(record, "c_wiki", "Wiki");
                }
            });
        }

        // Agrego la columna de Mantis
        if (this.opt_show_mantis) {
            this.columns.push({
                header: 'T',
                sortable: this.opt_column_sortable,
                width: 40,
                renderer: function(value, metaData, record, row, col, store, gridView){
                	return this.salidaHTML(record, "c_mantis", "Mantis");           
                }
            });
        }
	...
	salidaHTML: function(record, clase, texto) {

		cliente = record.data['tags'].toString().split(',')[0];
		if (this.opt_clientes_estandar.indexOf(cliente) == -1) {
			return;
		}

		if (clase === 'c_wiki') {
			imagen = '<img src="themes/canopsis/resources/images/lupa.png" alt="' + texto + '" />';
		} else {
			imagen = '<img src="themes/canopsis/resources/images/ticket.png" alt="' + texto + '" />';
		}
		var html = '<a href="#" class="' + clase +
			'" data-estado="' + record.data['state'] +
			'" data-host="' + record.data['component'] +
			'" data-servicio="' + record.data['resource'] +
			'" data-mensaje="' + record.data['output'] +
